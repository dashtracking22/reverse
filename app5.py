...
@app.route("/")
def index():
    return render_template("betkarma.html")
...
def get_odds(sport):
    ...
    try:
        ...
        for game in data:
            ...
            # Convert UTC to EST
            try:
                utc = datetime.strptime(game["commence_time"], "%Y-%m-%dT%H:%M:%SZ")
                est = utc.replace(tzinfo=pytz.utc).astimezone(pytz.timezone("US/Eastern"))
                game_time_str = est.strftime("%m/%d, %I:%M %p ET")
            except Exception:
                game_time_str = "Time TBD"

            results.append({
                "matchup": matchup,
                "commence_time_est": game_time_str,
                "opening": opening,
                "current": current,
                "movement": movement
            })
