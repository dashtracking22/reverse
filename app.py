...
def test_redis():
    ...
    # Save value to Redis
    save_url = f"{redis_url}/set/{key}"
    requests.post(save_url, headers=headers, json={"value": value})

    # Retrieve value from Redis
    get_url = f"{redis_url}/get/{key}"
    response = requests.get(get_url, headers=headers)

    retrieved = response.json().get("result", {}).get("value", None)
    return jsonify({"saved": value, "retrieved": retrieved})
